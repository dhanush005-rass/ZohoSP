        ...
        while (true) {
            ...
            switch (choice) {
                case 1:
                    ...
                    if (start <= end) {
                        for (int i = start; i <= end; i++) tollRoute.add(i);
                    } else {
                        for (int i = start; i < tollPoints.size(); i++) tollRoute.add(i);
                        for (int i = 0; i <= end; i++) tollRoute.add(i);
                    }

                    int totalToll = highway.calculateTollForRoute(tollRoute, vehicleType, isVIP);
                    System.out.println("Total Toll Paid: " + totalToll);

                    highway.processJourney(vehicleNumber, vehicleType, isVIP,
                            String.valueOf(start), String.valueOf(end), tollRoute);
                    break;
